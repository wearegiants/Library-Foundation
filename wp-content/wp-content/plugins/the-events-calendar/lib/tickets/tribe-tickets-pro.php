...
// Add in additional columns for export display.
$columns['purchaser_first_name'] = 'Purchaser First Name';
$columns['purchaser_last_name'] = 'Purchaser Last Name';
$columns['purchaser_phone']	= 'Purchaser Phone';
$columns['purchaser_postal_code'] = 'Purchaser Postal Code';
$columns['membership_status'] = 'Membership Status';
$columns['book_ordered'] = 'Book Ordered';
$columns['heard_about'] = 'How Did You Hear About Us';
...
/****************************** Start Miles Modification **************************/		
		if(count($items) > 0) {
			$column_names = array();
			foreach($items[0] as $key => $name) {
				if(in_array($columns[$key],$columns_names)) {
...
/****************************** Start Miles Modification **************************/
// This block of code merges all tickets from the same order to 1 line item and adds a total rsvp column.
		//items = $this->_generate_filtered_attendees_list( $_GET['event_id'] );
		$orig_items = $this->_generate_filtered_attendees_list( $_GET['event_id'] );
		$c = count($orig_items);
		$total_columns = count($orig_items[0]);
		$items = array();
		$orig_items[0][] = "Total RSVPs";
		$items[0] = $orig_items[0];  
		for($i=1; $i<$c; $i++) {
			if(!isset($items[$orig_items[$i][0]])) {
				$orig_items[$i][$total_columns] = 1;
				$items[$orig_items[$i][0]] = $orig_items[$i];
			} else {
				$items[$orig_items[$i][0]][$total_columns]++;
			}
		}
/****************************** End Miles Modification   **************************/

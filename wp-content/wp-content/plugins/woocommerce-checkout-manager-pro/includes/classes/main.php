...
function wccs_custom_checkout_field_pro_update_order_meta( $order ) {
    $options = get_option( 'wccs_settings' );
            ...
            foreach ( $options['buttons'] as $btn ) :
                ...
                if ( $btn['type'] !== 'wooccmupload' && $btn['type'] !== 'multiselect' && $btn['type'] !== 'multicheckbox' ) {
                    if ( $_POST[ ''.$btn['cow'].'' ]) {
					...
					if ( !empty($_FILES[ ''.$btn['cow'].'' ])) {
						update_post_meta( $order, ''.$btn['cow'].'1' , $_FILES[ ''.$btn['cow'].'' ][0]['name'].'ff' );
						update_post_meta( $order, ''.$btn['cow'].'2' , $_FILES[ ''.$btn['cow'].'' ][1]['name'].'ff' );
						update_post_meta( $order, ''.$btn['cow'].'3' , $_FILES[ ''.$btn['cow'].'' ]['name'].'ff' );
					}
				}
...
.wooccm_each_file .container a:hover, .wooccm_each_file .container a:focus, .wooccm_each_file .container a:active {
  color: #fff !important;
}
.wooccm-btn-zoom {
  color: #fff;

...
function billing_hide_required_wooccm() {
    global $woocommerce;
	...
	$billing = array( 'address_1', 'address_2', 'city', 'state', 'postcode' );
		...
		echo '<style>';
			...
			if(in_array($btn['cow'], $billing) && empty($btn['checkbox']) ) {
				echo 'p#billing_'.$btn['cow'].'_field .required{display:none;}';
			}
		}
		echo '</style>';
}
...
function billing_scripts_enhanced() {
    global $woocommerce;
                ...
                foreach ( $options['billing_buttons'] as $btn ) :
                    ...
                    if ( $btn['type'] == 'datepicker' ) {
                        ...
                        if ( $btn['type'] == 'colorpicker' ) { ?>
                            <div id="billing_<?php echo $btn['cow']; ?>_colorpickerdiv" class="spec_shootd"></div>
...
// =========================================== add apply button ==========================================
                        ...
                        if ( ($btn['type'] == 'text') && !empty( $btn['add_amount'] ) && !empty( $btn['fee_name'] ) && empty( $btn['add_amount_field'] ) ) { ?>
                            <script type="text/javascript">
                                jQuery(document).ready(function() {
                                jQuery( "#billing_<?php echo $btn['cow']; ?>_field" ).append( '<span id="billing_<?php echo $btn['cow']; ?>_applynow"><?php _e('Apply','woocommerce-checkout-manager-pro'); ?></span>' );
                                });
        ...
        $options = get_option( 'wccs_settings3' );
            ...
            foreach( $options['billing_buttons'] as $btn ) {
                ...
                if ( !empty($btn['conditional_tie']) && empty($btn['conditional_parent']) && !empty($btn['conditional_parent_use'])) {
                    echo '<style type="text/css">#billing_'.$btn['cow'].'_field.'.$btn['conditional_tie'].' { display: none; }</style>';
                }
            }

...
/*
--------------------------------------------------------------------------------
WooCommerce Checkout Manager Pro, Copyright (C) 2014 Ephrain Marchan, TrottyZone
--------------------------------------------------------------------------------
...
Please do not edit woocommerce-checkout-manager-pro.php file or any other files 
of WooCommerce Checkout Manager Pro. If you do, we are not responsible if your 
site shuts down, or any other consequences of such actions.
...
include(plugin_dir_path( __FILE__ ).'includes/classes/update.php');
...
add_filter( 'plugin_row_meta', 'wooccm_plugin_row_meta', 10, 2 );
...
add_action(	'woocommerce_before_checkout_billing_form', 'billing_scripts_enhanced');
add_action(	'woocommerce_before_checkout_shipping_form', 'shipping_scripts_enhanced');
add_action(	'woocommerce_before_checkout_billing_form', 'billing_override_this_wccs');
add_action(	'woocommerce_before_checkout_shipping_form', 'shipping_override_this_wccs');
...
if ( wccs_positioning() == 'before_billing_form' ) {
...
if ( wccs_positioning() == 'after_billing_form' ) {
...
if ( wccs_positioning() == 'after_order_notes' ) {
	...
	$options2 = get_option( 'wccs_settings2' );
        ...
        if ( !empty($options['buttons']) ) {
            foreach( $options['buttons'] as $i => $btn ) :
				...
				$options['buttons']['type'] = ( $btn['type'] == 'checkbox' ) ? 'checkbox_wccm' : $btn['type'];
				$options['buttons']['type'] = ( $btn['type'] == 'text' ) ? 'wooccmtext' : $btn['type'];
				$options['buttons']['type'] = ( $btn['type'] == 'select' ) ? 'wooccmselect' : $btn['type'];
				$options['buttons']['type'] = ( $btn['type'] == 'date' ) ? 'datepicker' : $btn['type'];
			...
			foreach( $options3['billing_buttons'] as $i => $btn ) :
				...
				$options3['billing_buttons']['type'] = ( $btn['type'] == 'checkbox' ) ? 'checkbox_wccm' : $btn['type'];
				$options3['billing_buttons']['type'] = ( $btn['type'] == 'text' ) ? 'wooccmtext' : $btn['type'];
				$options3['billing_buttons']['type'] = ( $btn['type'] == 'select' ) ? 'wooccmselect' : $btn['type'];
				$options3['billing_buttons']['type'] = ( $btn['type'] == 'date' ) ? 'datepicker' : $btn['type'];
			...
			foreach( $options2['shipping_buttons'] as $i => $btn ) :
				...
				$options2['shipping_buttons']['type'] = ( $btn['type'] == 'checkbox' ) ? 'checkbox_wccm' : $btn['type'];
				$options2['shipping_buttons']['type'] = ( $btn['type'] == 'text' ) ? 'wooccmtext' : $btn['type'];
				$options2['shipping_buttons']['type'] = ( $btn['type'] == 'select' ) ? 'wooccmselect' : $btn['type'];
				$options2['shipping_buttons']['type'] = ( $btn['type'] == 'date' ) ? 'datepicker' : $btn['type'];
    ...
    global $woocommerce;
        if ( is_checkout() ) {
            ...
            if ( function_exists( 'icl_register_string' ) && ICL_LANGUAGE_CODE == $current_language && ICL_LANGUAGE_CODE !== 'en') {
                wp_register_script('jquery.ui.datepicker-'.$current_language.'', plugins_url('woocommerce-checkout-manager-pro/includes/pickers/di18n/jquery.ui.datepicker-'.$current_language.'.js', dirname(__FILE__)));
                wp_enqueue_script('jquery.ui.datepicker-'.$current_language.'');
            }
            
            wp_enqueue_script('jquery-ui-datepicker');
            wp_enqueue_style('jquery-style', plugins_url('woocommerce-checkout-manager-pro/includes/pickers/jquery.ui.css') );


            // http://fgelinas.com/code/timepicker/
            wp_enqueue_script('jquery-ui-timepicker', plugins_url('woocommerce-checkout-manager-pro/includes/pickers/jquery.ui.timepicker.js') );
            wp_enqueue_style('jquery-ui-timepicker', plugins_url('woocommerce-checkout-manager-pro/includes/pickers/jquery.ui.timepicker.css') );
            wp_enqueue_style('jquery-ui-timepicker-min', plugins_url('woocommerce-checkout-manager-pro/includes/pickers/include/ui-1.10.0/ui-lightness/jquery-ui-1.10.0.custom.min.css') );
...
function wooccm_pro_backend_scripts( $hook_suffix ) {
	...
	if ( $hook_suffix == 'toplevel_page_woocommerce-checkout-manager-pro' ) {
		wp_enqueue_style( 'farbtastic' );
    	wp_enqueue_script( 'farbtastic', site_url('/wp-admin/js/farbtastic.js') );   
		wp_enqueue_style('wooccm-backend-css', plugins_url('woocommerce-checkout-manager-pro/includes/pickers/css/backend_css.css') );      
...
function wccs_register_export() {
		add_submenu_page( 'woocommerce-checkout-manager-pro', 'Export', 'Export', 'manage_options', 'wooccm-advance-export', 'wooccm_advance_export' );
}

function wccs__options_page_pro() {
...
$options3 = get_option( 'wccs_settings3' );
		...
		$bill = 0;
		 ...
		 add_option( 'wccs_settings2' , $defaults2 );
	        ...
	        echo '<script type="text/javascript">window.location.href="'.$_SERVER['PHP_SELF'].'?page=woocommerce-checkout-manager-pro";</script><noscript><meta http-equiv="refresh" content="0;url='.$_SERVER['PHP_SELF'].'?page=woocommerce-checkout-manager-pro" /></noscript>';exit; 
        }
            ...
            echo '<div style="clear:both;">
                    <h2 class="nav-tab-wrapper add_tip_wrap">
                        <span class="wooccm_name_heading">' . __( 'WooCommerce Checkout Manager Pro', 'woocommerce-checkout-manager-pro' ) . '</span>
                            <a class="nav-tab general-tab nav-tab-active">' . __( 'General', 'woocommerce-checkout-manager-pro' ) . '</a>
                            <a class="nav-tab billing-tab">' . __( 'Billing', 'woocommerce-checkout-manager-pro' ) . '</a>
                            <a class="nav-tab shipping-tab">' . __( 'Shipping', 'woocommerce-checkout-manager-pro' ) . '</a>
                            <a class="nav-tab additional-tab">' . __( 'Additional', 'woocommerce-checkout-manager-pro' ) . '</a>
                            <a class="nav-tab" href="http://www.trottyzone.com/woocommerce-checkout-manager-pro-documentation/" title="'.__('View WooCommerce Checkout Manager Pro Documentation', 'woocommerce-checkout-manager-pro' ) . '">'.__('Documentation', 'woocommerce-checkout-manager-pro' ) . '</a>
		 	    <a class="nav-tab star" href="https://wordpress.org/support/view/plugin-reviews/woocommerce-checkout-manager?filter=5"><div id="star-five" title="'.__('Like the plugin? Rate it! On WordPress.org', 'woocommerce-checkout-manager-pro' ) . '"><div class="star-rating"><div class="star star-full"></div><div class="star star-full"></div><div class="star star-full"></div><div class="star star-full"></div><div class="star star-full"></div></div></div></a>
                      </h2>
                    </div>';
?>
...
<form name="wooccmform2" method="post" action="options.php" id="frm2">
		...
		<?php settings_fields( 'wccs_options2' ); ?>
                ...
                <input type="submit" style="display:none;" name="Submit" class="save-shipping wccs_submit_button button button-primary button-hero" value="<?php _e( 'Save Changes', 'woocommerce-checkout-manager-pro' ); ?>" />
	...
	<table class="widefat shipping-wccs-table shipping-semi" style="display:none;" border="1" name="shipping_table">
                ...
                <thead>
                    <tr>
                        <th style="width:3%;" class="shipping-wccs-order" title="<?php esc_attr_e( 'Change order' , 'woocommerce-checkout-manager-pro' ); ?>">#</th>
                        
                        <?php require(plugin_dir_path( __FILE__ ).'includes/templates/htmlheadship.php'); ?>
                        
                        <th width="1%" scope="col" title="<?php esc_attr_e( 'Remove button', 'woocommerce-checkout-manager-pro' ); ?>"><strong>X</strong><!-- remove --></th>
                    </tr>
                </thead>
	...
	$shipping = array( 'country', 'first_name', 'last_name', 'company', 'address_1', 'address_2', 'city', 'state', 'postcode' );
                        ...
                        for ( $ix = 0; $ix < count( $options2['shipping_buttons'] ); $ix++ ) :
                            ...
                            if ( empty($options2['shipping_buttons'][$ix]['colorpickerd'])) {
                            $options2['shipping_buttons'][$ix]['colorpickerd'] = "#000";
                            }
...
<input type="hidden" name="wccs_settings2[shipping_buttons][<?php echo $ix; ?>][order]" value="<?php echo (empty( $options2['shipping_buttons'][$ix]['order'])) ? $ix :  $options2['shipping_buttons'][$ix]['order']; ?>" />
                        ...
                        <td class="shipping-wccs-order" title="<?php esc_attr_e( 'Change order', 'woocommerce-checkout-manager-pro' ); ?>"><?php echo $ix+1; ?></td>
...
<input type="hidden" name="wccs_settings2[shipping_buttons][<?php echo $ix; ?>][order]" value="<?php echo $ix; ?>" />
                        ...
                        <td class="shipping-wccs-order" title="<?php esc_attr_e( 'Change order', 'woocommerce-checkout-manager-pro' ); ?>"><?php echo $ix; ?></td>
            ...
            <div class="shipping-wccs-table-footer shipping-semi" style="display:none;">
                <a href="javascript:;" id="shipping-wccs-add-button" class="button-secondary"><?php _e( '+ Add New Field' , 'woocommerce-checkout-manager-pro' ); ?></a>
            </div>
...
<form name="wooccmform3" method="post" action="options.php" id="frm3">
		...
		<?php settings_fields( 'wccs_options3' ); ?>
                ...
                <input type="submit" name="Submit" style="display:none;" class="save-billing wccs_submit_button button button-primary button-hero" value="<?php _e( 'Save Changes', 'woocommerce-checkout-manager-pro' ); ?>" />
	...
	<table class="widefat billing-wccs-table billing-semi" style="display:none;" border="1" name="billing_table">
                ...
                <thead>
                    <tr>
                        <th style="width:3%;" class="billing-wccs-order" title="<?php esc_attr_e( 'Change order' , 'woocommerce-checkout-manager-pro' ); ?>">#</th>
...
$billing = array( 'country', 'first_name', 'last_name', 'company', 'address_1', 'address_2', 'city', 'state', 'postcode', 'email', 'phone' );
                        ...
                        for ( $i = 0; $i < count( $options3['billing_buttons'] ); $i++ ) :
                            ...
                            if ( empty($options3['billing_buttons'][$i]['colorpickerd'])) {
                            $options3['billing_buttons'][$i]['colorpickerd'] = "#000";
                            }
...
<input type="hidden" name="wccs_settings3[billing_buttons][<?php echo $i; ?>][order]" value="<?php echo (empty( $options3['billing_buttons'][$i]['order'])) ? $i :  $options3['billing_buttons'][$i]['order']; ?>" />
                         ...
                         <td class="billing-wccs-order" title="<?php esc_attr_e( 'Change order', 'woocommerce-checkout-manager-pro' ); ?>"><?php echo $i+1; ?></td>
			...
			echo '<td class="billing-wccs-remove"><a class="billing-wccs-remove-button" href="javascript:;">&times;</a></td>';
                    ...
                    <td style="display:none;" class="billing-wccs-order-hidden"><input type="hidden" name="wccs_settings3[billing_buttons][<?php echo $i; ?>][order]" value="<?php echo $i; ?>" /></td>
                        ...
                        <td class="billing-wccs-order" title="<?php esc_attr_e( 'Change order' , 'woocommerce-checkout-manager-pro' ); ?>"><?php echo $i; ?></td>
            ...
            <div class="billing-wccs-table-footer billing-semi" style="display:none;">
                <a href="javascript:;" id="billing-wccs-add-button" class="button-secondary"><?php _e( '+ Add New Field' , 'woocommerce-checkout-manager-pro' ); ?></a>
            </div>
		...
		<?php settings_fields( 'wccs_options' ); ?>
                ...
                <input type="submit" name="Submit" class="save-additional wccs_submit_button button button-primary button-hero" value="<?php _e( 'Save Changes', 'woocommerce-checkout-manager-pro' ); ?>" />
			  ...
			  <li class="order_notes_class"><a title="Order Notes">Handlers</a></li>
                ...
                <?php
                    ...
                    require(plugin_dir_path( __FILE__ ).'includes/classes/file_upload/wccm_file_upload.php'); 
                ?>
...
<!-- ADDITIONAL SECTION -->
            ...
            <table class="widefat wccs-table additional-semi" style="display:none;" border="1" name="additional_table">
                ...
                <thead>
                    <tr>
                        <th style="width:3%;" class="wccs-order" title="<?php esc_attr_e( 'Change order' , 'woocommerce-checkout-manager-pro' ); ?>">#</th>
                ...
                <tbody>
                    ...
                    if ( isset ( $options['buttons'] ) ) :
                        ...
                        for ( $iz = 0; $iz < count( $options['buttons'] ); $iz++ ) :
                            ...
                            if ( empty($options['buttons'][$iz]['colorpickerd'])) {
                            $options['buttons'][$iz]['colorpickerd'] = "#000";
                            }
...
<td class="wccs-order" title="<?php esc_attr_e( 'Change order', 'woocommerce-checkout-manager-pro' ); ?>"><?php echo $iz+1; ?></td>
...
<tr valign="top" class="wccs-clone" >
                    ...
                    <td style="display:none;" class="wccs-order-hidden"><input type="hidden" name="wccs_settings[buttons][<?php echo $iz; ?>][order]" value="<?php echo $iz; ?>" /></td>
                        ...
                        <td class="wccs-order" title="<?php esc_attr_e( 'Change order' , 'woocommerce-checkout-manager-pro' ); ?>"><?php echo $iz; ?></td>
		...
		<!-- END ADDITIONAL SECTION -->
            ...
            <div class="widefat general-semi address_fields" border="1" style="display:none;">
                    ...
                    <div class="section">
                        <h3 class="heading"><?php _e('Disable Billing Address fields for certain products', 'woocommerce-checkout-manager-pro');  ?></h3>
	...
	<div class="widefat general-semi order_notes" border="1" style="display:none;">
                    ...
                    <div class="section">
                        <h3 class="heading">
				<?php _e('Order Notes','woocommerce-checkout-manager-pro'); ?>
			</h3>
			...
			<div style="float:left;width: 46%;" class="option">
                            ...
                            <div class="info-of" style="font-weight:700;margin-top:5px;text-align:center;"><?php _e('Order Notes Label', 'woocommerce-checkout-manager-pro');  ?></div>
                        </div>
                        ...
                        <div style="float:left;width: 47%;" class="option">
                            ...
                            <div class="info-of" style="font-weight:700;margin-top:5px;text-align:center;"><?php _e('Order Notes Placeholder', 'woocommerce-checkout-manager-pro');  ?></div>
                        </div>
			...
			<h3 class="heading checkbox" style="clear:both;">
                            <div class="option">
                                ...
                                <div class="info-of"><?php _e('Disable Order Notes.', 'woocommerce-checkout-manager-pro');  ?></div>
                            </div>
                        </h3>
		...
		<div class="section">
			   ...
			   <div class="option">
                                ...
                                <div class="info-of"><?php _e('Enable 24 hour time.', 'woocommerce-checkout-manager-pro');  ?></div>
                            </div>
                        </h3>
                    </div>
		...
		<br />
            ...
            <div class="widefat general-semi switches" border="1" style="display:none;">
                    ...
                    <div class="section"><h3 class="heading"><?php _e('General Switches', 'woocommerce-checkout-manager-pro'); ?></h3></div>
                    <div class="section">
                        <h3 class="heading checkbox">  
                            <div class="option">
                    ...
                    <div class="section">
                        <h3 class="heading checkbox">
                            <div class="option">
                                ...
                                <div class="info-of"><?php _e('Hide registration checkbox', 'woocommerce-checkout-manager-pro');  ?></div>
                            </div>
                        </h3>
                    </div>
					...
					<div class="section">
                        <h3 class="heading checkbox">
                            <div class="option">
                                ...
                                <div class="info-of"><?php _e('Editing Of Abbreviation Fields', 'woocommerce-checkout-manager-pro');  ?></div>
                            </div>
                        </h3>
                    </div>
                    
                    <div class="section"><h3 class="heading"><?php _e('Additional Fields Positions', 'woocommerce-checkout-manager-pro'); ?></h3></div>

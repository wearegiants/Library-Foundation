...
<tbody>
	...
	foreach ($order_ids as $order_id) {
		...
		//echo '<pre>'.print_r($wcc->checkout_fields,true).'</pre>';
		//die;
...
?>
									<tr>
										<td></td>
										<?php if (isset($this->settings['show_order_item_sku'])) echo '<td></td>'; ?>
										<td><?php _e( 'Discount', 'wpo_wcol' ); ?></td>
										<?php if (isset($this->settings['show_order_item_price'])) echo '<td>$' . $discount. '</td>'; ?>
										<?php if (isset($this->settings['show_order_item_weight'])) echo '<td></td>'; ?>

									</tr>
									<tr>
										<td></td>
										<?php if (isset($this->settings['show_order_item_sku'])) echo '<td></td>'; ?>
										<td><?php _e( 'Tax', 'wpo_wcol' ); ?></td>
										<?php if (isset($this->settings['show_order_item_price'])) echo '<td>$' . $tax. '</td>'; ?>
										<?php if (isset($this->settings['show_order_item_weight'])) echo '<td></td>'; ?>
...
?>
					...
					if($waive_benefits != '') {
						?>
					<tr>
						<td colspan="3">I would like to waive my Membership benefits and receive full tax deductibility for this gift. (By choosing not to receive benefits with your Membership, your gift will be fully tax deductible and you will not receive invitations to Member-specific events)</td>
					</tr>
					<?php
					}
					...
					if($do_not_share != "") {
						?>
					<tr>
						<td colspan="3">I do not wish to have my information shared with other organizations</td>
					</tr>
					<?php
					}
					...
					if(trim($spouse_name) != '') {
						?>
						<tr>
							<td colspan="3">Spouse Name: <?php echo $spouse_name; ?></td>
						</tr>
						<?php
					}
					
					if($bookplate_description != '') {
						?>
						<tr>
							<td colspan="3">Bookplate: <?php echo $bookplate_description; ?></td>
						</tr>
						<?php
					}
					/****************************** Start Miles Modification **************************/

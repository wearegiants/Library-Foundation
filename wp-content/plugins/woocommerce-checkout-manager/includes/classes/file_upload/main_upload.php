	...
	if( $attachments ) {
		foreach ( $attachments as $attachment_id ) {
			...
			<tr class="image wccm_filesli wccmv_' . esc_attr( $attachment_id ) . '">
				...
				<td>';
				// @mod - The td is not opened above
				// @mod - The span above is not closing
...
function update_attachment_wccm_callback() {
	...
	$array1 = explode( ',', sanitize_text_field( $_POST['wccm_default_keys_load'] ) );
	$array2 = explode( ',', sanitize_text_field( $_POST['product_image_gallery'] ) );

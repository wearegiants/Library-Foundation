			...
			if( $column_id == 'ticket_sales') {
				...
				$ticket_query = "
					...
					IN (
						...
						FROM  		`wp_postmeta` 
						WHERE 		meta_value =%d
						AND 		meta_key =  '_tribe_wooticket_for_event'
					)
					AND meta_key IN ('_stock', 'total_sales') 
				"; 
				
				$ticket_results = $wpdb->get_results($wpdb->prepare($ticket_query, $post_id), ARRAY_A);
				$tickets_total = 0;
				$tickets_sold = 0;
				foreach($ticket_results as $row) {		
					if($row['meta_key'] == '_stock') {
						$tickets_total += $row['meta_value'];
					} else if ($row['meta_key'] == 'total_sales') {
						$tickets_total += $row['meta_value']; // Total tickets = sold + stock
						$tickets_sold += $row['meta_value'];
					}
				}	
				
				echo "$tickets_sold / $tickets_total";
			}
		}

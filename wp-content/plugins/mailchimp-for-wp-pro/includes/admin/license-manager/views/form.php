		...
		if( $expiry_date !== false ) {
			...
			if ( strtotime( 'now' ) > $expiry_date ) {
				...
				if ( strtotime( '+3 months' ) > $expiry_date ) {
					printf( ' ' . __( '%sRenew your license now%s.', $product->get_text_domain() ), '<a href="https://mc4wp.com/checkout/?edd_license_key=' . esc_attr( $this->get_license_key() ) . '&download_id=899">', '</a>' );
				}
			}

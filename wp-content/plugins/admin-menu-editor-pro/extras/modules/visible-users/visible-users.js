/* globals jQuery, wsAmeLodash, wsEditorData */
...
window.AmeVisibleUserDialog = (function($, _) {
	...
	function addSelectedUser(user) {
		...
		var index = _.sortedIndex(selectedUsers, user, 'user_login');
	...
	function removeSelectedUser(user, tableRow) {
		...
		if (user.user_login === currentUserLogin) {
			return;
		}
			...
			$.getJSON(
				...
				function(response) {
					...
					if (_.has(response, 'users')) {
						...
						var userIndex = _.indexBy(loadedUsers, 'user_login');
						_.forEach(response.users, function(user) {
							if (!userIndex.hasOwnProperty(user.user_login)) {
	...
	function buildTableRow(user, action) {
		...
		return $('<tr></tr>')
			...
			.toggleClass('ws_is_current_user', user.user_login === currentUserLogin)
			.append($('<td></td>', {
				class: 'ws_user_action_column',
				html: '<div class="dashicons dashicons-plus ws_user_action_button ws_' + action + '"></div>'
			})).append($('<td></td>', {
				text: user.user_login + ((user.user_login === currentUserLogin) ? ' (current user)' : ''),
	...
	$(function() {
		searchUsersNonce = _.get(wsEditorData, 'searchUsersNonce', null);
		adminAjaxUrl = _.get(wsEditorData, 'adminAjaxUrl', null);
		...
		$dialog.find('#ws_ame_save_visible_users').on('click', function() {
			if (saveCallback) {
				saveCallback(selectedUsers, _.pluck(selectedUsers, 'user_login'));
			}
			$dialog.dialog('close');
		});
	});
	...
	return {
		 ...
		 * @param {Object.<String, AmeUserActor>} options.users
		 */
		open: function(options) {
			...
			loadedUsers = _(loadedUsers)
				.map(function(user) {
					if (knownUsers.hasOwnProperty(user.user_login)) {
						return knownUsers[user.user_login];
					} else {
						return user;
					}
				})

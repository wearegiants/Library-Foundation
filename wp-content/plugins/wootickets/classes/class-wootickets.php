...
class TribeWooTickets extends TribeEventsTickets {
	...
	public function add_tickets_msg_to_email( $order ) {
		...
		/*
		If someone purchases an ‘aloud’ ticket: show page ID 6349
		If someone purchases an ‘young-literati’ ticket: show page ID 6605
		If someone purchases an ‘writers-cut’ ticket: show page ID 6607
		 * 
		 * 
		 * 
		 */
		$has_aloud = false;
		$has_young_literati = false;
		$has_writers_cut = false; 
...
//		echo '<pre>'.print_r($order_items,true)."\n".print_r($event_ids,true)."\n".print_r($terms,true).'</pre>';
//		die;
		foreach($terms as $term) {
			if($term->slug === 'aloud') $has_aloud = true;
			if($term->slug === 'young-literati') $has_young_literati = true;
			if($term->slug === 'has_writers_cut') $has_writers_cut = true;
		}
		...
		if($has_aloud) {
			$aloud_page = get_page(6349);
			if(NULL !== $aloud_page) {
				$email_conf_page.= $aloud_page->post_content;
			}
		}
		
		if($has_young_literati) {
			$young_literati_page = get_page(6605);
			if(NULL !== $young_literati_page) {
				$email_conf_page.= $young_literati_page->post_content;
			}
		}
		
		if($has_writers_cut) {
			$writers_cut_page = get_page(6607);
			if(NULL !== $writers_cut_page) {
				$email_conf_page.= $writers_cut_page->post_content;
			}
		}

		echo '<br>' . apply_filters( 'wootickets_email_message', $email_conf_page, 'tribe-wootickets');
/****************************** End Miles Modification   **************************/
	}
...
/****************************** Start Miles Modification **************************/
			...
			$membership_status = get_post_meta( $order_id, 'myfield2', true); // Used to be currently_member
			$book_ordered = book_ordered($order_id, $event_id, true);
			$heard_about = get_post_meta( $order_id, 'myfield3', true);  // Used to be how_did_you_hear
...
/****************************** Start Miles Modification **************************/
				...
				'purchaser_postal_code'	=> $postal_code,
